...
export default function Analytics() {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      ...
      try {
        const { data, error } = await supabase.from("bookings").select("*").eq("restaurant_id", restaurantId);
        ...
        const revenue = bookings.filter(b => b.status === "completed").reduce((acc, b) => acc + (b.total_amount || 0), 0);
        ...
        const byDay = last7Days.map(dateStr => {
          const count = bookings.filter(b => b.date && b.date.startsWith(dateStr)).length;
        ...
        setTimeSlotData(
          Object.entries(slots).map(([time, count]) => ({ time, count }))
